...
{
    public class GetFirstDailyTransactionQuery : IRequest<DailyTransactionVm>
    {
        public Guid CustomerId { get; set; }
        public int FinanceYear { get; set; }
        public class GetFirstDailyTransactionQueryHandler : IRequestHandler<GetFirstDailyTransactionQuery, DailyTransactionVm>
        {
            private readonly IApplicationDbContext _context;
            public GetFirstDailyTransactionQueryHandler(IApplicationDbContext context)
            {
                _context = context;
            }
            public async Task<DailyTransactionVm> Handle(GetFirstDailyTransactionQuery request, CancellationToken cancellationToken)
            {
                var bondSno = _context.DailyTransactions.Where(d => d.CustomerId == request.CustomerId && d.DailyTransactionYear == request.FinanceYear).Select(d => d.DailyTransactionBondSNo).Min();
