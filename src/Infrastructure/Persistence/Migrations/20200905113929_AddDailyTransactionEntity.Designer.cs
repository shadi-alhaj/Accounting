// <auto-generated />
using System;
using Accounting.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Accounting.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20200905113929_AddDailyTransactionEntity")]
    partial class AddDailyTransactionEntity
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Accounting.Domain.Entities.Bond", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("BondNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_BOND_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("BondNameEn")
                        .HasColumnName("ACC_BOND_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<int>("BondUserId")
                        .HasColumnName("ACC_BOND_USER_ID")
                        .HasColumnType("int");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_BOND_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_BOND_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_BOND_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("IntialSNo")
                        .HasColumnName("ACC_BOND_INTIAL_SNO")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_BOND_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_BOND_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_BOND_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.ToTable("ACC_BONDS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.Customer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Address")
                        .HasColumnName("ACC_CUSTOMER_ADDRESS")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("City")
                        .HasColumnName("ACC_CUSTOMER_CITY")
                        .HasColumnType("nvarchar(100)")
                        .HasMaxLength(100);

                    b.Property<string>("Country")
                        .HasColumnName("ACC_CUSTOMER_COUNTRY")
                        .HasColumnType("nvarchar(100)")
                        .HasMaxLength(100);

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_CUSTOMER_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_CUSTOMER_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("CustomerId")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_CUSTOMER_ID")
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CustomerNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_CUSTOMER_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("CustomerNameEn")
                        .HasColumnName("ACC_CUSTOMER_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("Email")
                        .HasColumnName("ACC_CUSTOMER_EMAIL")
                        .HasColumnType("nvarchar(100)")
                        .HasMaxLength(100);

                    b.Property<string>("FaxNo")
                        .HasColumnName("ACC_CUSTOMER_FAX_NO")
                        .HasColumnType("nvarchar(20)")
                        .HasMaxLength(20);

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_CUSTOMER_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_CUSTOMER_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_CUSTOMER_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MobileNo1")
                        .HasColumnName("ACC_CUSTOMER_MOBILE_NO1")
                        .HasColumnType("nvarchar(14)")
                        .HasMaxLength(14);

                    b.Property<string>("MobileNo2")
                        .HasColumnName("ACC_CUSTOMER_MOBILE_NO2")
                        .HasColumnType("nvarchar(14)")
                        .HasMaxLength(14);

                    b.Property<string>("PhoneNo")
                        .HasColumnName("ACC_CUSTOMER_PHONE_NO")
                        .HasColumnType("nvarchar(20)")
                        .HasMaxLength(20);

                    b.Property<string>("TaxNo")
                        .HasColumnName("ACC_CUSTOMER_TAX_NO")
                        .HasColumnType("nvarchar(20)")
                        .HasMaxLength(20);

                    b.HasKey("Id");

                    b.ToTable("ACC_CUSTOMERS");

                    b.HasData(
                        new
                        {
                            Id = new Guid("988cc910-2246-433b-a382-ae5454a855aa"),
                            Address = "Macca St.",
                            City = "Amman",
                            Country = "Jordan",
                            Created = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CustomerId = 0,
                            CustomerNameAr = "مكتب المدير",
                            CustomerNameEn = "Almodeer Office",
                            IsActive = false,
                            MobileNo1 = "0795980824",
                            TaxNo = "123456"
                        });
                });

            modelBuilder.Entity("Accounting.Domain.Entities.DailyTransaction", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("BondId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_BOND_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_DAILY_TRANSACTION_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_DAILY_TRANSACTION_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("DailyTransactionBondSNo")
                        .HasColumnName("ACC_DAILY_TRANSACTION_BOND_SNO")
                        .HasColumnType("int");

                    b.Property<decimal>("DailyTransactionCreditAmount")
                        .HasColumnName("ACC_DAILY_TRANSACTION_CREDIT_AMOUNT")
                        .HasColumnType("decimal(18,3)");

                    b.Property<DateTime>("DailyTransactionDate")
                        .HasColumnName("ACC_DAILY_TRANSACTION_DATE")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("DailyTransactionDebitAmount")
                        .HasColumnName("ACC_DAILY_TRANSACTION_DEBIT_AMOUNT")
                        .HasColumnType("decimal(18,3)");

                    b.Property<string>("DailyTransactionDescription")
                        .IsRequired()
                        .HasColumnName("ACC_DAILY_TRANSACTION_DESCRIPTION")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<int>("DailyTransactionIdByCustomer")
                        .HasColumnName("ACC_DAILY_TRANSACTION_ID_BY_CUSTOMER")
                        .HasColumnType("int");

                    b.Property<int>("DailyTransactionMonth")
                        .HasColumnName("ACC_DAILY_TRANSACTION_MONTH")
                        .HasColumnType("int");

                    b.Property<int>("DailyTransactionYear")
                        .HasColumnName("ACC_DAILY_TRANSACTION_YEAR")
                        .HasColumnType("int");

                    b.Property<Guid>("DetailAccountId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_DETAIL_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("GeneralLedgerId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_GL_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_DAILY_TRANSACTION_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_DAILY_TRANSACTION_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_DAILY_TRANSACTION_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("MainAccountId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_MAIN_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("TotalAccountId")
                        .HasColumnName("ACC_DAILY_TRANSACTION_TOTAL_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("BondId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("DetailAccountId");

                    b.HasIndex("GeneralLedgerId");

                    b.HasIndex("MainAccountId");

                    b.HasIndex("TotalAccountId");

                    b.ToTable("ACC_DAILY_TRANSACTIONS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.DetailAccount", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("DetailAccountIdByCustomer")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_ID_BY_CUSTOMER")
                        .HasColumnType("int")
                        .HasMaxLength(250);

                    b.Property<string>("DetailAccountNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_DETAIL_ACCOUNT_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("DetailAccountNameEn")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<Guid>("GeneralLeadgerId")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_GL_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_DETAIL_ACCOUNT_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("MainAccountId")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_MAIN_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("TotalAccountId")
                        .HasColumnName("ACC_DETAIL_ACCOUNT_TOTAL_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.HasIndex("GeneralLeadgerId");

                    b.HasIndex("MainAccountId");

                    b.HasIndex("TotalAccountId");

                    b.ToTable("ACC_DETAIL_ACCOUNTS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.FinanceYear", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_FINANCE_YEAR_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_FINANCE_YEAR_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_FINANCE_YEAR_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_FINANCE_YEAR_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_FINANCE_YEAR_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_FINANCE_YEAR_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Year")
                        .HasColumnName("ACC_FINANCE_YEAR_YEAR")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.ToTable("ACC_FINANCE_YEARS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.GeneralLedger", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_GL_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_GL_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_GL_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("GLNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_GL_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<int>("GlIdByCustomer")
                        .HasColumnName("ACC_GL_ID_BY_CUSTOMER")
                        .HasColumnType("int")
                        .HasMaxLength(250);

                    b.Property<string>("GlNameEn")
                        .HasColumnName("ACC_GL_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_GL_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_GL_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_GL_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.ToTable("ACC_GENERAL_LEDGERS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.MainAccount", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_MAIN_ACCOUNT_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_MAIN_ACCOUNT_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_MAIN_ACCOUNT_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("GeneralLeadgerId")
                        .HasColumnName("ACC_MAIN_ACCOUNT_GL_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_MAIN_ACCOUNT_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_MAIN_ACCOUNT_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_MAIN_ACCOUNT_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("MainAccountIdByCustomer")
                        .HasColumnName("ACC_MAIN_ACCOUNT_ID_BY_CUSTOMER")
                        .HasColumnType("int")
                        .HasMaxLength(250);

                    b.Property<string>("MainAccountNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_MAIN_ACCOUNT_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("MainAccountNameEn")
                        .HasColumnName("ACC_MAIN_ACCOUNT_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.HasIndex("GeneralLeadgerId");

                    b.ToTable("ACC_MAIN_ACCOUNTS");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.TodoItem", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("Created")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Done")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ListId")
                        .HasColumnType("int");

                    b.Property<string>("Note")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.Property<DateTime?>("Reminder")
                        .HasColumnType("datetime2");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.HasKey("Id");

                    b.HasIndex("ListId");

                    b.ToTable("TodoItems");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.TodoList", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Colour")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Created")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.HasKey("Id");

                    b.ToTable("TodoLists");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.TotalAccount", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Created")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_CRT_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatedBy")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_CRT_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("CustomerId")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_CUSTOMER_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("GeneralLeadgerId")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_GL_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("ACC_TOTAL_ACCOUNT_IS_ACTIVE")
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsClose")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_IS_CLOSE")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("LastModified")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_UPD_DATE")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastModifiedBy")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_UPD_BY_USR_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("MainAccountId")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_MAIN_ACCOUNT_ID")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("TotalAccountIdByCustomer")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_ID_BY_CUSTOMER")
                        .HasColumnType("int")
                        .HasMaxLength(250);

                    b.Property<string>("TotalAccountNameAr")
                        .IsRequired()
                        .HasColumnName("ACC_TOTAL_ACCOUNT_NAME_AR")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.Property<string>("TotalAccountNameEn")
                        .HasColumnName("ACC_TOTAL_ACCOUNT_NAME_EN")
                        .HasColumnType("nvarchar(250)")
                        .HasMaxLength(250);

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.HasIndex("GeneralLeadgerId");

                    b.HasIndex("MainAccountId");

                    b.ToTable("ACC_TOTAL_ACCOUNTS");
                });

            modelBuilder.Entity("Accounting.Infrastructure.Identity.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("IdentityServer4.EntityFramework.Entities.DeviceFlowCodes", b =>
                {
                    b.Property<string>("UserCode")
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<string>("ClientId")
                        .IsRequired()
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<DateTime>("CreationTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("Data")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasMaxLength(50000);

                    b.Property<string>("DeviceCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<DateTime?>("Expiration")
                        .IsRequired()
                        .HasColumnType("datetime2");

                    b.Property<string>("SubjectId")
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.HasKey("UserCode");

                    b.HasIndex("DeviceCode")
                        .IsUnique();

                    b.HasIndex("Expiration");

                    b.ToTable("DeviceCodes");
                });

            modelBuilder.Entity("IdentityServer4.EntityFramework.Entities.PersistedGrant", b =>
                {
                    b.Property<string>("Key")
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<string>("ClientId")
                        .IsRequired()
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<DateTime>("CreationTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("Data")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasMaxLength(50000);

                    b.Property<DateTime?>("Expiration")
                        .HasColumnType("datetime2");

                    b.Property<string>("SubjectId")
                        .HasColumnType("nvarchar(200)")
                        .HasMaxLength(200);

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.HasKey("Key");

                    b.HasIndex("Expiration");

                    b.HasIndex("SubjectId", "ClientId", "Type");

                    b.ToTable("PersistedGrants");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(128)")
                        .HasMaxLength(128);

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("Accounting.Domain.Entities.Bond", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("Bonds")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.DailyTransaction", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Bond", "Bond")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("BondId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.DetailAccount", "DetailAccount")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("DetailAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.GeneralLedger", "GeneralLedger")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("GeneralLedgerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.MainAccount", "MainAccount")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("MainAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.TotalAccount", "TotalAccount")
                        .WithMany("DailyTransactions")
                        .HasForeignKey("TotalAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.DetailAccount", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("DetailAccounts")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.GeneralLedger", "GeneralLedger")
                        .WithMany("DetailAccounts")
                        .HasForeignKey("GeneralLeadgerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.MainAccount", "MainAccount")
                        .WithMany("DetailAccounts")
                        .HasForeignKey("MainAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.TotalAccount", "TotalAccount")
                        .WithMany("DetailAccounts")
                        .HasForeignKey("TotalAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.FinanceYear", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("FinanceYears")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.GeneralLedger", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("GeneralLedgers")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.MainAccount", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("MainAccounts")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.GeneralLedger", "GeneralLedger")
                        .WithMany("MainAccounts")
                        .HasForeignKey("GeneralLeadgerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.TodoItem", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.TodoList", "List")
                        .WithMany("Items")
                        .HasForeignKey("ListId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Accounting.Domain.Entities.TotalAccount", b =>
                {
                    b.HasOne("Accounting.Domain.Entities.Customer", "Customer")
                        .WithMany("TotalAccounts")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.GeneralLedger", "GeneralLedger")
                        .WithMany("TotalAccounts")
                        .HasForeignKey("GeneralLeadgerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Accounting.Domain.Entities.MainAccount", "MainAccount")
                        .WithMany("TotalAccounts")
                        .HasForeignKey("MainAccountId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Accounting.Infrastructure.Identity.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Accounting.Infrastructure.Identity.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Accounting.Infrastructure.Identity.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Accounting.Infrastructure.Identity.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
